...
class AnalysisCaenCCD:
	...
	def LoadTree(self):
		if not os.path.isdir(self.dir):
			ExitMessage('The directory {d} does not exist! Exiting...'.format(d=self.dir), os.EX_DATAERR)
		root_files = glob.glob('{d}/*.root'.format(d=self.dir))
		if len(root_files) == 1:
			self.tree_name = root_files[0].split('/')[-1].split('.root')[0]
			self.root_file = ro.TFile(root_files[0])
		elif len(root_files) == 0:
			ExitMessage('There is no root file inside the directory {d}. Exiting...'.format(d=self.dir), os.EX_DATAERR)
		else:
			...
			if file_to_open in root_files:
				self.tree_name = file_to_open.split('/')[-1].split('.root')[0]
				self.root_file = ro.TFile(file_to_open)
			else:
				ExitMessage('The given file: {f} is not part of the given list. Exiting...'.format(f=file_to_open), os.EX_DATAERR)
		self.root_tree = self.root_file.Get(self.tree_name)

	def LoadPickles(self):
		if self.tree_name != '':
			if os.path.isdir(self.dir):
				if os.path.isfile('{d}/{f}.settings'.format(d=self.dir, f=self.tree_name)):
					with open('{d}/{f}.settings'.format(d=self.dir, f=self.tree_name), 'rb') as pklsets:
						self.settings = pickle.load(pklsets)
				if os.path.isfile('{d}/{f}.signal_ch'.format(d=self.dir, f=self.tree_name)):
					with open('{d}/{f}.signal_ch'.format(d=self.dir, f=self.tree_name)) as pklsign:
						self.signal_ch = pickle.load(pklsign)
				if os.path.isfile('{d}/{f}.trigger_ch'.format(d=self.dir, f=self.tree_name)):
					with open('{d}/{f}.trigger_ch'.format(d=self.dir, f=self.tree_name)) as pkltrig:
						self.trigger_ch = pickle.load(pkltrig)
				if os.path.isfile('{d}/{f}.veto_ch'.format(d=self.dir, f=self.tree_name)):
					with open('{d}/{f}.veto_ch'.format(d=self.dir, f=self.tree_name)) as pklveto:
						self.veto_ch = pickle.load(pklveto)
				elif os.path.isfile('{d}/{f}.veto'.format(d=self.dir, f=self.tree_name)):
					with open('{d}/{f}.veto'.format(d=self.dir, f=self.tree_name)) as pklveto:
						self.veto_ch = pickle.load(pklveto)
...
def main():
	parser = OptionParser()
	parser.add_option('-d', '--dir', dest='dir', default='.', type='string', help='Directory containing the run files')
